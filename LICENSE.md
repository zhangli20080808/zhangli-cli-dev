...
import-local的实际用途是当全局和项目中同时该命令时优先执行项目中的命令。以zhangli-cli-dev为例子，全局和项目中都有该命令，但是版本不一样，当我们执行publish发布的时候会优先执行项目中的命令，使用的版本也是项目中的版本，当你下载了该命令是一定是想在本地使用的，如果想进行调试的话本地安装的版本需要通过 node_modules/.bin/xxx 进行调用，比较麻烦
