...
- 脚手架核心框架
  ...
  * 本地缓存体系 - 存在意义？在脚手架注册过程中，将某一些配置信息，在本地做缓存，会涉及到文件缓存。一般来说 2 种方案，缓存到本地文件中，或者环境变量中
- 初始化体系
- 标准 git 操作体系
...
- 发布体系
  ...
  1. 外部依赖 - 依赖资源体系，比如所有的发布都可以发布到资源体系。
     构建的产物要上传到 oss，同事 oss 要同步到 cdn（oss 不能直接加速），保证访问速度
...
- Websocket
  ...
  - 云发布
  - 发布模式
  1. 外部依赖，远程的缓存体系。比如说，创建了一个构建任务，多人构建，可能会是并行操作，不是单线程。每条任务走 redis 存储，云构建的过程，也需要将代码下载到本地，进行一系列操作，服务端也需要提供缓存。因为云构建的产物是没地方放置的
...
- 准备阶段
  ...
  7. 检查是否为最新版本 - 提示更新
- 命令注册
  核心 - 基于 Commander 实现脚手架的命令注册和执行过程开发
  主要内容
...
- commander - 通过 commander 做命令注册,lerna 使用的是 yargs，对比学习
