...
- 脚手架核心框架
  ...
  * 本地缓存体系 - 存在意义？在脚手架注册过程中，将某一些配置信息，在本地做缓存，会涉及到文件缓存。一般来说2种方案，缓存到本地文件中，或者环境变量中
- 初始化体系
- 标准 git 操作体系
...
- Websocket
  ...
  - 云发布
  - 发布模式
  1. 外部依赖，远程的缓存体系。比如说，创建了一个构建任务，多人构建，可能会是并行操作，不是单线程。每条任务走redis存储，云构建的过程，也需要将代码下载到本地，进行一系列操作，服务端也需要提供缓存。因为云构建的产物是没地方放置的
